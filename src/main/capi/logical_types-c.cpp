...
static bool AssertLogicalTypeId(duckdb_logical_type_const type, duckdb::LogicalTypeId type_id) {
	if (!type) {
		return false;
	}
	const auto &logical_type = *(reinterpret_cast<const duckdb::LogicalType *>(type));
...
static bool AssertInternalType(duckdb_logical_type_const type, duckdb::PhysicalType physical_type) {
	if (!type) {
		return false;
	}
	const auto &logical_type = *(reinterpret_cast<const duckdb::LogicalType *>(type));
...
duckdb_logical_type duckdb_create_list_type(duckdb_logical_type_const type) {
	...
	try {
		duckdb::LogicalType *logical_type =
		    new duckdb::LogicalType(duckdb::LogicalType::LIST(*reinterpret_cast<const duckdb::LogicalType *>(type)));
...
duckdb_logical_type duckdb_create_array_type(duckdb_logical_type_const type, idx_t array_size) {
	...
	try {
		duckdb::LogicalType *ltype = new duckdb::LogicalType(
		    duckdb::LogicalType::ARRAY(*reinterpret_cast<const duckdb::LogicalType *>(type), array_size));
...
duckdb_logical_type duckdb_create_union_type(duckdb_logical_type_const DUCKDB_API_CONST *member_types_p,
...
duckdb_logical_type duckdb_create_struct_type(duckdb_logical_type_const DUCKDB_API_CONST *member_types_p,
                                              const char *DUCKDB_API_CONST *member_names, idx_t member_count) {
	if (!member_types_p || !member_names) {
		return nullptr;
	}
...
duckdb_logical_type duckdb_create_map_type(duckdb_logical_type_const key_type, duckdb_logical_type_const value_type) {
	if (!key_type || !value_type) {
		return nullptr;
	}
	try {
		duckdb::LogicalType *mtype = new duckdb::LogicalType(
		    duckdb::LogicalType::MAP(*reinterpret_cast<const duckdb::LogicalType *>(key_type),
		                             *reinterpret_cast<const duckdb::LogicalType *>(value_type)));
		return reinterpret_cast<duckdb_logical_type>(mtype);
	} catch (...) {
		return nullptr;
	}
...
char *duckdb_enum_dictionary_value(duckdb_logical_type_const type, idx_t index) {
	...
	const auto &logical_type = *(reinterpret_cast<const duckdb::LogicalType *>(type));
	const auto &vector = duckdb::EnumType::GetValuesInsertOrder(logical_type);
...
duckdb_logical_type duckdb_map_type_key_type(duckdb_logical_type_const type) {
	...
	const auto &mtype = *(reinterpret_cast<const duckdb::LogicalType *>(type));
...
duckdb_state duckdb_register_logical_type(duckdb_connection_const connection, duckdb_logical_type_const type,
	...
	const auto &base_type = *reinterpret_cast<const duckdb::LogicalType *>(type);
	...
	try {
		const auto con = reinterpret_cast<const duckdb::Connection *>(connection);
